...
static void oem_emmcupdate(char *cmd_parameter, char *response)
{
	...
	if (!strcmp(cmd_parameter, "writebl2")) {
		...
		env_name = "crc32-bl2";
	} else if (!strcmp(cmd_parameter, "writefip_vlpv4")) {
		cmd_ret = write_to_eMMC_bootpart(FIP_ADD_300_SAVE_TO_EMMC, &crc_val);
		env_name = "crc32-fip";
	} else if (!strcmp(cmd_parameter, "writefip_vlpv3")) {
		cmd_ret = write_to_eMMC_bootpart(FIP_ADD_320_SAVE_TO_EMMC, &crc_val);
		env_name = "crc32-fip";
	} else {
